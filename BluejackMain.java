...
public class BluejackMain{
	public static void main (String[]args){
		GameDeck[] deckGame=new GameDeck[40];/////////////CREATE GAMEDECK Array
		int a=1;
		int b=1;
		int c=1;
		for(int i=0;i<10;i++){
		...
		GameDeck[]deckPlayer2=new GameDeck[5];//(%20)FLİP
		for(int i=0;i<4;i++){
		deckPlayer2[i]=deckPlayer1[r.nextInt(48)];
		}
		deckPlayer2[4]=new GameDeck(0,"+/-");
		GameDeck[]deckPlayer3=new GameDeck[5];//(%20)DOUBLE 
		for(int i=0;i<4;i++){
			deckPlayer3[i]=deckPlayer1[r.nextInt(48)];
		}
		deckPlayer3[4]=new GameDeck(0,"Double");
		GameDeck[]unHandHuman=new GameDeck[10];//UNCOMPLETED HUMAN DECK
		for(int i=0;i<5;i++){
		...
		GameDeck[] humanDeck=new GameDeck[5];    // COMPLETED HUMAN DECK
		for(int i=0;i<5;i++){
			humanDeck[i]=unHandHuman[r.nextInt(10)];
		}
		GameDeck[]unHandPC=new GameDeck[10];//UNCOMPLETED PC DECK
		for(int i=0;i<5;i++){
		...
		GameDeck[] pcDeck=new GameDeck[5];// COMPLETED PC DECK
		for(int i=0;i<5;i++){
	...
	///////////////////////////////////////////////////////FUNCTİONS////////////////////////////////////////////////////////////////////////
		...
		switch(a){
			...
			int b=sc.nextInt()-1;
			while(arr[b].getNumber()==0){
				System.out.println("AYNI KART 2 KEZ KULLANILAMAZ");
		      System.out.println("Enter card number");
			   b=sc.nextInt()-1;
			}
			d=arr[b].getNumber();
			arr[b]=new GameDeck(0,"KULLANILDI");
		}
		return d;
	}
	...
	public static int fonkSwitchPC(int a, int c,GameDeck[]arr){
		...
		switch(a){
			...
			d=arr[b].getNumber();
			System.out.println("PC "+arr[b].getNumber()+","+arr[b].getColour()+" Kartını Kullandı");
			arr[b]=new GameDeck(0,"KULLANILDI");
